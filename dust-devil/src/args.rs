...
pub fn get_help_string() -> &'static str {
    concat!(
        ...
        "Examples:\n",
        "\n",
        "Starts the server listening for SOCKS5 clients at all IPv4 addresses on port 1080 and Sandstorm clients at all ",
        "IPv6 addresset on port 2222 (the ports are implicit), writing logs to a logs.txt file and creating a new admin ",
        "user called \"pedro\" with password \"1234\":\n",
        "    dust-devil -o logs.txt -l 0.0.0.0 -m [::] -u @pedro:1234\n",
        "\n",
        "Starts the server with the default sockets listening for incoming SOCKS5 clients, but only allows incoming ",
        "management connections from 127.0.0.1:3443. The buffer size for clients is set to 4096 bytes, logging and ",
        "metrics are disabled:\n",
        "    dust-devil -m 127.0.0.1:3443 -b 4k -d -s\n",
        "\n",
        "Starts the server with the default listening sockets, diasbles \"noauth\" authentication (to force clients to ",
        "authenticate with a username and password), and creates three users: Admin user 'Nicolás' with password ",
        "'#nicorules', regular user 'Greg:orio with password 'holus', and regular user '#tade0' with password 'tadaa':\n",
