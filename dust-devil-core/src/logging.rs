...
pub enum LogEvent {
    ...
    ClientFailedToConnectToRemote(u64),
    ClientConnectedToRemote(u64, SocketAddr),
...
impl fmt::Display for LogEvent {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ClientFailedToConnectToRemote(client_id) => write!(f, "Client {client_id} failed to connect to destination, sending error response"),
            Self::ClientConnectedToRemote(client_id, socket_address) => write!(f, "Client {client_id} successfully established connection to destination at {socket_address}"),
