...
struct Controller<W: AsyncWrite + Unpin + 'static> {
    ...
    user_watch: broadcast::Sender<(UserNotificationType, String, UserRole)>,
...
async fn add_user_task<W: AsyncWrite + Unpin + 'static>(
    ...
    let popup = match maybe_result {
        Some(AddUserResponse::Ok) => {
            let _ = rc.user_watch.send((UserNotificationType::Registered, username, role));
            rc.close_popup();
            return;
        }
        Some(error_response) => {
...
impl<W: AsyncWrite + Unpin + 'static> AddUserPopup<W> {
    pub fn new(
        ...
        let (controller, close_receiver) = Controller::new(Rc::clone(&redraw_notify), manager, user_watch, popup_sender);
