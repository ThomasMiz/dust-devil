...
pub fn get_help_string() -> &'static str {
    concat!(
        ...
        "Examples:\n",
        "\n",
        "Connects to the server at 192.168.1.1:2222 (the port is implicit), logs in with user 'admin' password 'admin', ",
        "and requests three consecutive meow pings:\n",
        "    sandstorm -x 192.168.1.1 -c admin:admin -w -w -w\n",
        "\n",
        "Connects to the server at 10.4.20.1:8900, logs in with user 'pedro' password '1234', then requests adding the admin ",
        "user 'pedro' with password '1234', updates the role of user 'marcos' to regular (the password is not changed), ",
        "deletes the user 'josé', and finally lists all the users:\n",
        "    sandstorm -x 10.4.2.1:8900 -c pedro:1234 -u @pedro:1234 -p #marcos -d josé -t\n",
        "\n",
        "Connects to the server at localhost:2222 (implicit) with user 'admin' and requests the current buffer size and ",
        "metrics:\n",
        ...
        "\n",
        "Connects to the server at localhost:2222 (implicit) with user 'admin', requests the list of listening SOCKS5 ",
        "sockets, and then leaves the connection open streaming server events and printing them to stdout until manually ",
        "closed (with Ctrl-C):\n",
        "    sandstorm -c admin:admin -l -o\n",
        "\n",
        "Connects to the server at localhost:2222 (implicit) with user 'admin', sends three meow pings and then opens the ",
